...
class TasksOfChild(APIView):
    ...
    def get(self, request, pk, format=None):
        try:
            tasks = Task.objects.filter(child__exact=pk, status__lte = 2)
...
class TasksOfParent(APIView):            
    ...
    def get(self,request,pk, format=None):
        try:
            children = Child.objects.filter(parent__exact=pk)
            for child in children:
                task = Task.objects.filter(child__exact = child__id, status__lte = 2)
...
class RewardsOfChild(APIView):
    ...
    def get(self, request, pk, format=None):
        try:
            reward = Reward.objects.filter(child__exact=pk)
...
class RewardsOfParent(APIView):            
    ...
    def get(self,request,pk, format=None):
        try:
            ...
            for child in children:
                reward = Reward.objects.filter(child__exact = child__id)
